...
//			sprintf(file,"0:\\%0.4d-%0.2d\\%0.2d-%0.2d.txt",timer.w_year,timer.w_month,timer.hour,timer.min);     //输入文件路径
		res = f_open(&parafile, file, FA_OPEN_ALWAYS|FA_WRITE);     //此处注意文件打开模式
		if(res) return res;
	  res = f_lseek(&parafile, parafile.fsize);       //偏移出文件空间
		f_write (&parafile, buffer, 44, &br);  ////形参的数组为写入的内容指针
...
//		sprintf(file,"0:\\%0.4d-%0.2d\\%0.2d-%0.2d.txt",2017,2,2,10);     //输入文件路径
		res = f_open(&parafile, "password.txt", FA_OPEN_ALWAYS|FA_WRITE);     //此处注意文件打开模式
		if(res) return res;
//	  res = f_lseek(&parafile, parafile.fsize);       //偏移出文件空间

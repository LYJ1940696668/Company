...
static const GUI_WIDGET_CREATE_INFO _aDlgWidgets[] = {
  ...
  { TEXT_CreateIndirect,      "Return",        GUI_ID_TEXT0,       10,  50, 100,  15, TEXT_CF_HCENTER},
  { BUTTON_CreateIndirect,    "Return",        GUI_ID_BUTTON0,     10,  65, 100,  20 },
  ...
  switch (pMsg->MsgId) {
    ...
    case WM_NOTIFY_PARENT:
      if (pMsg->Data.v == WM_NOTIFICATION_VALUE_CHANGED) {
        ...
        hItem = WM_GetDialogItem(hDlg, GUI_ID_CHECK0);
        if (pMsg->hWinSrc == hItem) {
					if(_AutoMode)BUTTON_SetPressed(_ahWin[BUTTON0], 0);
...
//   GUI_CURSOR_Show();
  ...
  _hFrameWin = FRAMEWIN_CreateEx(0, 0, 799, 479, 0, WM_CF_SHOW, 0, 0, "", &_cbCallbackFramewin);
  ...
  while (1) {
    if (_AutoMode) {
      ...
      if ((Cnt % 400) == 0) {
		...
		else if(BUTTON_IsPressed(_ahWin[BUTTON0])==1)
		{
			GUI_EndDialog(_hDlg, 0);
			GUI_EndDialog(hDlgOptions, 0);
			FRAMEWIN_Delete(_hFrameWin);
			return;
		}
    /* Wait a while */
    GUI_Delay(10);
